...
def wrap_html_for_print(html_content: str, umlauf: str, df_admin=None) -> str:
    auftragnehmer = "-"
    zusatz_info = ""

    if df_admin is not None:
        auftragnehmer = get_admin_value(df_admin, "Auftragnehmer")
        if not auftragnehmer or auftragnehmer.strip() in ["", "-"]:
            auftragnehmer = "Unbekannter Auftragnehmer"

        # ✅ Spezialfall für Meyer van der Kamp
        if auftragnehmer.strip().lower() == "meyer van der kamp":
            zusatz_info = "<div>Meyer van der Kamp GmbH &amp; Co. KG – Jadesand<br><a href='mailto:info@jadesand.de'>info@jadesand.de</a></div>"
...
def generate_export_html(
    ...
    zeitzone = get_admin_value(df_admin, "Zeitzone")
    zeitzone_kurz = "UTC" if zeitzone == "UTC" else "Lokal"
    ...
    <div style="display: flex; gap: 16px; margin-bottom: 1em;">
        ...
        <div style="border-bottom: 1px solid #ccc; padding-bottom: 2px; margin-bottom: 6px; font-weight: 600;">Umlauf & Zeitraum</div>
            <table style="width: 100%;">
                ...
                <tr><td>Beginn:</td><td style="text-align: right;">{format_time(umlauf_row.get("Start Leerfahrt"), zeitzone)} ({zeitzone_kurz})</td></tr>
                <tr><td>Ende:</td><td style="text-align: right;">{format_time(umlauf_row.get("Ende"), zeitzone)} ({zeitzone_kurz})</td></tr>
            </table>
        </div>
    ...
    <div style="display: flex; gap: 16px; margin-bottom: 1em;">
      <!-- Zeiten -->
        <div style="flex: 40;">
            <div style="border-bottom: 1px solid #ccc; padding-bottom: 2px; margin-bottom: 6px;">Zeiten ({zeitzone_kurz}) </div>
            <table style="">
